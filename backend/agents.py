...
Gemini fallback: when Groq hits its daily token cap (HTTP 429), make_writer()
and make_editor() can be called with a Gemini LLM so the pipeline still publishes.
"""
...
def groq_fast_llm(max_tokens: int = 1500) -> LLM:
...
def groq_quality_llm(max_tokens: int = 5000) -> LLM:
...
def gemini_llm(max_tokens: int = 5000) -> LLM:
    """gemini-2.0-flash — fallback when Groq daily token cap is exhausted."""
    return LLM(
...
def make_economist(llm: LLM | None = None) -> Agent:
    return Agent(
        ...
        llm=llm if llm is not None else groq_fast_llm(max_tokens=1200),
        max_iter=2,
        verbose=False,
...
def make_writer(llm: LLM | None = None) -> Agent:
    return Agent(
        ...
        llm=llm if llm is not None else groq_quality_llm(max_tokens=5000),
        max_iter=2,
        verbose=False,
        allow_delegation=False,
    )
...
def make_editor(llm: LLM | None = None) -> Agent:
    return Agent(
        ...
        llm=llm if llm is not None else groq_quality_llm(max_tokens=5500),
        max_iter=2,
        verbose=False,
        allow_delegation=False,
    )
...
# Module-level instances (Groq) — kept for backward compatibility with tasks.py imports.
economist_agent = make_economist()
writer_agent = make_writer()
editor_agent = make_editor()
